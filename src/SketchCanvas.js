...
class SketchCanvas extends React.Component {
    ...
    addShape(config) {
        if (config) {
            ...
            const shapeName = config.shapeName || "-";
            const pictureDetails = config.pictureDetails;
    ...
    selectShapeById(id) {
        if (id) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.selectShapeById, [id]);
        }
    }

    moveSelectedShape(value) {
        if (value) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.moveSelectedShape, [
                { value }
            ]);
        }
    }

    scaleSelectedShape(value) {
        if (value) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.scaleSelectedShape, [
                { value }
            ]);
        }
    }

    rotateSelectedShape(value) {
        if (value) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.rotateSelectedShape, [
                { value }
            ]);
        }
    }

    moveShapeById(id, value) {
        if (id && value) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.moveShapeById, [
                id,
                { value }
            ]);
        }
    ...
    rotateShapeById(id, value) {
        if (id && value) {
            UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.rotateShapeById, [
                id,
                { value }
            ]);
        }
    ...
    deleteShapeById(id) {
        UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.deleteShapeById, [id]);
    }
    ...
    changeSelectedShapeText(newText) {
        UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.changeShapeText, [
            newText
        ]);
    }

    save(imageType, transparent, folder, filename, includeImage, includeText, cropToImageSize) {
        UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNSketchCanvas.Commands.save, [
            imageType,
            folder,
            filename,
            transparent,
